...
watcher 和 dep 的关系：一个watcher对应n个dep，一个dep对应n个watcher => 多对多
...
当我们更新数据后，vue执行update方法执行渲染watcher，渲染组件。我们希望能拿到更新后的数据。
...
**定义nextTick，src/util/next-tick.js**

调用nextTick时，现价回调函数暂存起来，等同步代码执行完后在执行。
...
组件更新时，渲染watcher通过队列控制成一起更新，src/observer/watcher.js
...
**通过队列结合nextTick实现异步更新，src/observer/schedular.js**
